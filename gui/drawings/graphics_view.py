...
import storage
from gui.drawings.event_handlers import Mover, LineDrawer, Deleter, Chooser, PointDrawer
...
class GraphicsView(QGraphicsView):
    ...
    def __init__(self, event_bus):
        ...
        self.task_result_handlers = {
            ...
            'delete_point': self.on_point_delete,
            'add_point': self.on_point_add,
    ...
    @block_processing
    def launch_add_point(self, point_1):
        self.event_bus.dispatch(Event(name='add_point', point_1=point_1))
    ...
    @block_processing
    def launch_delete_point(self, point_id):
        print('point id', point_id)
        self.event_bus.dispatch(Event(name='delete_point', point_id=point_id))
    ...
    def on_point_add(self, task_result):
    ...
    def on_line_delete(self, task_result):
        ...
        # point_id = task_result
            
    def on_point_delete(self, task_result):
        point_id = task_result.get('point_id', None)
        print('task', task_result)
        if point_id is None:
            raise RuntimeError('invalid_result')
        self.scene().removeItem(self.points.get(point_id))
        # line_dict = self.lines.get(point_id, None)
        # p1_id = line_dict['p1_id']
        # p2_id = line_dict['p2_id']
        # point1_handle = self.points.get(p1_id)
        # point2_handle = self.points.get(p2_id)
    ...
    def on_constraint_clicked(self, event):
        ...
        for elem in objects:
            if elem['type'] == 'line':
                ...
                item = self.points[elem['obj']]['point']
                tmp = self.scene().addEllipse(item.rect(), pen=get_pen(Qt.green, 4))
            self.tmpobj.append(tmp)
    ...
    def redraw_scene(self):
        ...
        for key, value in self.storage.points.items():
            print(key, value)
            print(value.x(), value.y())
            point_handle = self.points.get(key)  # line should definitely exist
            x = value.x()
            y = value.y()

            point_handle.setRect(x - self.POINT_RADIUS,
                                  y - self.POINT_RADIUS,
                                  self.POINT_RADIUS ** 2,
                                  self.POINT_RADIUS ** 2)
